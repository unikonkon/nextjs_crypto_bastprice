...
export default function FTX({                         // รับค่าเข้ามา
  symbol,
  urlSocket,
  exchange,
}) {
  ...
  const get_price_from_socket = (event) => {            //ประกาศตัวแปรสำหรับดึงข้อมูล 
    try {                                               //เขียน try เพื่อ กำหนดโค้ตที่จะรัน
        ...
        set_price(data.data.bid);                       //เซ็ตค่า set_price จาก ตัวแปร data เข้าถึง data เข้าถึง bid
    } catch (err) {
    ...
    try {
      ...
      client.onopen = function () {                     //ใช้ onopen เพื่อเตรียมข้อมูลก่อนที่จะส่งข้อมูลไป
        client.send(JSON.stringify({                    //ส่งข้อมูลไปเป็น JSON ใช้JSON.stringify() 
            op: "subscribe",      
            channel: "ticker",
            market: `${symbol.toUpperCase()}-PERP`,    //ในข้อมูลที่ส่ง มี ` ` เพื่อที่จะใส่ข้อมูลที่รับค่ามาจาก symbol
          }))
      };
